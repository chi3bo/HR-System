...
import { adminLetter } from 'src/app/shared/interfaces/admin';
...
@Component({
  ...
  styleUrls: ['./admin-letters.component.css']
})
export class AdminLettersComponent {
  ...
  ngOnInit(): void {
    ...
    this._AdminService.getAllLetters().subscribe({
      next: (data) => {
  ...
  actionRequest(orderId: number, action: boolean, details: string, oneItem: HTMLElement) {
    ...
    this._AdminService.LettersAction(orderId, action, details).subscribe({
